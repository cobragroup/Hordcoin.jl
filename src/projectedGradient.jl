...
"""
	project_to_constraints(data, joint_probability::Array{Float64}, marginals; model::Model = Model(SCS.Optimizer)) -> Vector{Float64}

Project a flattened candidate distribution `data` onto the affine set of joint tables whose specified **marginals** match those of `joint_probability`.
Uses a second‑order cone program (least‑squares distance) via JuMP.

- `data`: Flattened probabilities (same length as `joint_probability`).
- `joint_probability`: N‑dimensional joint table providing target marginals.
...
"""
function project_to_constraints(data, joint_probability::Array{<:AbstractFloat}, marginals::Vector{Tuple}; model::Model = Model(SCS.Optimizer))::Array{Float64}
...
"""
	project_with_model(data, joint_probability::Array{Float64}, marginals, optimiser::SCS.Optimizer) -> Vector{Float64}

Shorthand to call `project_to_constraints` using SCS.
"""
function project_with_model(data, joint_probability::Array{<:AbstractFloat}, marginals::Vector{Tuple}, optimiser::SCS.Optimizer)::Array{Float64}
	project_to_constraints(data, joint_probability, marginals; model = Model(typeof(optimiser)))
end

"""
	project_with_model(data, joint_probability::Array{Float64}, marginals, optimiser::MosekTools.Optimizer) -> Vector{Float64}

Shorthand to call `project_to_constraints` using Mosek.
"""
function project_with_model(data, joint_probability::Array{<:AbstractFloat}, marginals::Vector{Tuple}, optimiser::MosekTools.Optimizer)::Array{Float64}
	project_to_constraints(data, joint_probability, marginals; model = Model(typeof(optimiser)))
end
...
"""
function descent(data, marginals::Vector{Tuple}; iterations = 1000, optimiser::MathOptInterface.AbstractOptimizer)::EMResult
	step = 0.01
	flat = vec(data)
